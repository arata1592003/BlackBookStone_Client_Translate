...
export const BookDetailTabs = ({
  ...
  if (!mounted) {
    return <section className="w-full mt-6 h-40 bg-surface-card animate-pulse rounded-lg" />;
  }
      ...
      <Tabs defaultValue={defaultTab} className="w-full">
        <TabsList className="grid w-full grid-cols-3 bg-surface-card border border-border-default rounded-t-lg rounded-b-none h-auto p-0 overflow-hidden">
          {TABS.map((tabItem) => (
            <TabsTrigger
              key={tabItem.id}
              value={tabItem.id}
              className="py-3 text-sm md:text-lg font-medium text-text-secondary data-[state=active]:bg-surface-raised data-[state=active]:text-primary rounded-none border-r border-border-default last:border-r-0 transition-all"
            >
              {tabItem.label}
            </TabsTrigger>
          ))}
        ...
        <div className="bg-surface-card border-x border-b border-border-default rounded-b-lg p-4 min-h-[200px]">
          <TabsContent value="intro" className="mt-0 outline-none">
            <div className="text-text-secondary text-base leading-relaxed font-roboto font-normal whitespace-pre-wrap">
