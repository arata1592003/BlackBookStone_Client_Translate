...
export const ChapterNewestList = ({ slug, newestChapterList }: Props) => {
  return (
    <section className="flex flex-col w-[900px] items-start justify-center relative flex-[0_0_auto] rounded-sm overflow-hidden border border-solid border-white">
      ...
      {newestChapterList.map((chapter) => (
        <Link
          ...
          className="flex items-start justify-between pl-2.5 pr-5 py-2.5 relative self-stretch w-full flex-[0_0_auto] border-b [border-bottom-style:solid] border-white no-underline text-text-secondary hover:bg-white/10"
        >
