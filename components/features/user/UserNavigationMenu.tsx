...
export const UserNavigationMenu = ({ isMobile = false }: UserNavigationMenuProps) => {
  ...
  const menuItems: MenuItem[] = [
    ...
    { id: "thong-tin", label: "Thông tin cá nhân", href: "/tai-khoan/thong-tin" },
  ...
  return (
    ...
    >
      ...
      <div className="gap-2.5 pl-2.5 pr-0 py-0 w-full flex flex-col relative self-stretch">
        <ul className="flex-col items-start gap-2 flex-1 grow flex relative" role="menu">
          {menuItems.map((item) => {
            const isActive = pathname === item.href || (item.id === "ban-lam-viec" && pathname === "/user");
            return (
              <li key={item.id} className="items-start gap-1 self-stretch w-full flex-[0_0_auto] flex relative" role="none">
                <Button
                  variant="ghost"
                  className={cn(
